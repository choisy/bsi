...
```{r}
required <- c("dplyr", "lubridate", "magrittr", "purrr", "readxl", "stringr", "survival", "tibble", "tidyr")
...
This function transforms a "Yes/No" variable into a boolean one, dealing with
missing values:
...
charlson <- data2 %>% 
  ...
  mutate(charlson     = select_if(., is.numeric) %>% rowSums(),
         charlson_cat = cut(charlson, c(-Inf, 0, 2, 4, Inf)) %>% as.character()) # not sure whether this should be here or later (depends on how many time it's used)
```
...
data2 %<>% 
  select(PatientNo, hospital, Sex, age, otherhosp, NKQ, tracheostomy,
         CLBSI_risk, ALBSI_risk, apache, charlson_cat, Tetanus, Pneumonia, sepsis,
...
```{r}
data3 <- data2 %>% 
  select(habsi, Sex, age, hospital, apache, charlson_cat, sepsis, Tetanus, CNS,
...
```{r}
data5 <- data2 %>% 
  select(death90, Sex, age, hospital, apache, charlson_cat, sepsis, Tetanus, CNS,
