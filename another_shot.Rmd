...
The following function transform a continuous variable into a categorical variable
with equal amount of data in each category:
...
n_and_percentage <- function(var) {
  a <- as.numeric(table(data4[[var]], data4$habsi))
...
```{r}
x <- "age"
m <- coxph(Surv(time2bsi, habsi) ~ data4[[x]], data4)
```
...
```{r}
nb_lev <- sapply(variables, function(x) length(unique(data4[[x]])))
```
...
```{r}
cbind(numbers, calc_c_uni(cox_coef), calc_c_uni(cox_p1), calc_p_uni(cox_p2), calc_c_multi(m), calc_p_multi1(m), calc_p_multi2(m)) %>% 
...
```{r}
cbind(numbers, calc_c_uni(cox_coef), calc_p_uni(cox_p2), calc_c_multi(m), calc_p_multi2(m)) %>% 
  as.data.frame(., row.names(.), stringsAsFactors = FALSE)
```
