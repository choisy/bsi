...
```{r}
a <- readRDS("protocol.VARI.rds")
```

```{r}
data2 %>% 
  select(PatientNo, Tetanus, VARI) %>% 
  left_join(a, "PatientNo") %>% 
  mutate(IsTetanus = IsTetanus == "Yes") %>% 
  filter(is.na(IsTetanus)) %>% 
  pull(Tetanus) %>% 
  table()
#  drop_na() %>% 
#  mutate(TetComp = Tetanus - IsTetanus, VariComp = VARI.x - VARI.y) %>% 
#  filter()
```
...
plotevent <- function(a, ylim = 0:1, col_line = "black", col_area = "grey", add = FALSE) {
  ...
  lwr <- a$lower
  upr <- a$upper
  sel <- sort(c(which(! near(diff(lwr), 0)), which(! near(diff(upr), 0))) + 1)
  ...
  if (! add)
    plot(a$time, 1 - a$surv, type = "s", xlab = "time (days)", ylab = "proportion dead", ylim = ylim)
  
  polygon(c(time2, rev(time2)), 1 - c(lwr2, rev(upr2)), col = col_area, border = NA)
  lines(a$time, 1 - a$surv, type = "s", col = col_line)
}
```
...
```{r}
a <- survfit(Surv(timeICU, deathICU) ~ habsi, data = data3)
```

Death at day 28:

```{r}
km28_2 <- survfit(Surv(time28, death28) ~ habsi, data = data3)
plot(km28_2, col = 3:2)
```
...
```{r}
km90_2 <- survfit(Surv(time90, death90) ~ habsi, data = data3)
plot(km90_2, col = 3:2)
```
