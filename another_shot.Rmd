...
```{r}
write.csv(data$MICBLD, "micbld.csv", quote = FALSE, row.names = FALSE)
```
...
```{r}
write.csv(data$DAILY1n2, "daily_icu.csv", quote = FALSE, row.names = FALSE)
```
...
data2 <- readRDS("death_day.full.rds") %>% 
  ...
  leftjoin(select(readRDS("icu_mortality.full.rds"), -IsTetanus)) %>% 
...
tmp %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "Word_tables/BSIincidence_rate.docx")
```
...
risk_factors(habsi, time2bsi) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "Word_tables/BSI_risk_factors.docx")
```
...
risk_factors(death90, DOD) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "Word_tables/mortality_risk_factors.docx")
```
