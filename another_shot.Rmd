...
for (i in c("VARI", "VAT", "VAP", "habsi_risk", "habsi", "ALBSI_risk", "ALBSI",
  ...
  make_table_1(i) %>% 
    print() %>% 
    flex2word(paste0("Table1", i, ".docx"))
}
```
...
tmp %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "BSIincidence_rate.docx")
```
...
risk_factors(habsi, time2bsi) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "BSI_risk_factors.docx")
```
...
risk_factors(death90, DOD) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "mortality_risk_factors.docx")
```
...
```{r eval = FALSE}
if (! require("kintr")) install_packages("knitr")
...
To execute the code (it takes 5-10''):
