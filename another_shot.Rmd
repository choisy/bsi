...
nb_path_bsi1 <- data$MICBLD %>% 
  filter(episode < 2) %>% 
  group_by(PatientNo) %>% 
  summarise(nb_path_bsi1 = length(first(strsplit(paste(unlist(pathogens), collapse = ", "), ", "))))
```
...
Let's see the frequencies of those which appear in more than one episode:
...
Let's now do the same but for individual pathogens instead of combinations:
...
And now only for first episodes:
