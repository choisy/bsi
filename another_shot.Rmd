...
```{r}
mean_ci_df <- function(x, var) {
  as.data.frame(t(mean_ci(x[[var]])))
}
```
...
lapply(groups, function(x) {
  data2 %>% 
    group_by(.[[x]]) %>% 
    group_modify(~ mean_ci_df(.x, "nb_cl_days"))
})
```

Any arterial line during ICU stay from randomisation:

```{r}
lapply2(groups, table2, "had_arterial")
```

Total number of central line days:

```{r}
lapply2(groups, quantile2, "nb_al_days")
```

Total number of arterial line patient-day:

```{r}
lapply(groups, function(x) {
  data2 %>% 
    group_by(.[[x]]) %>% 
    summarise(patientday = sum(nb_al_days, na.rm = TRUE))
})
```

Number of arterial line day per patient (mean, CI95%):

```{r}
lapply(groups, function(x) {
  data2 %>% 
    group_by(.[[x]]) %>% 
    group_modify(~ mean_ci_df(.x, "nb_al_days"))
})
```
...
```{r}
lapply2(groups, table2, "deathICUdis")
```

Mortality at ICU discharge, by tetanus status:

```{r}
data2 %>% 
  split(.$Tetanus) %>%
  lapply2(function(x) lapply2(groups, function(group) table(x[["deathICUdis"]], x[[group]]))) %>% 
  setNames(c("without", "with"))
```

Mortality at ICU discharge by site:

```{r}
data2 %>% 
  split(.$hospital) %>%
  lapply2(function(x) lapply2(groups, function(group) table(x[["deathICUdis"]], x[[group]]))) %>% 
  setNames(sort(unique(data2$hospital)))
```

Mortality at hospital discharge:

```{r}
lapply2(groups, table2, "deathHOSdis")
```

Mortality at hospital discharge, by tetanus status:

```{r}
data2 %>% 
  split(.$Tetanus) %>%
  lapply2(function(x) lapply2(groups, function(group) table(x[["deathHOSdis"]], x[[group]]))) %>% 
  setNames(c("without", "with"))
```

Mortality at hospital discharge by site:
...
```{r}
lapply2(groups, table2, "death28days")
```

Mortality at 28 days, by tetanus status:

```{r}
data2 %>% 
  split(.$Tetanus) %>%
  lapply2(function(x) lapply2(groups, function(group) table(x[["death28days"]], x[[group]]))) %>% 
  setNames(c("without", "with"))
```

Mortality at 28 days, by site:
...
```{r}
lapply2(groups, table2, "death90days")
```

Mortality at 90 days, by tetanus status:

```{r}
data2 %>% 
  split(.$Tetanus) %>%
  lapply2(function(x) lapply2(groups, function(group) table(x[["death90days"]], x[[group]]))) %>% 
  setNames(c("without", "with"))
```

Mortality at 90 days, by site:
...
```{r}
ht_theme_markdown <- function(ht, header_rows = 1, header_cols = 1, border_width = .8, border_color = grey(.75), strip = TRUE) {
...
```{r}
data2 %>% 
  select(PatientNo, hospital, Sex, age, otherhosp, NKQ, tracheostomy, apache,
         charlson_cat, Tetanus, Pneumonia, sepsis, CNS, COPD, CVA, Myo, otherpath,
         habsi_risk, habsi, CLBSI_risk, CLBSI, ALBSI_risk, ALBSI) %>% 
  mutate(habsi = ifelse(habsi, "HABSI", "non-HABSI")) %>% 
  sstable.baseline(hospital + Sex + age + otherhosp + NKQ + tracheostomy + apache +
                   charlson_cat + Tetanus + Pneumonia + sepsis + CNS + COPD + CVA +
                   Myo + otherpath ~ habsi,
  ...
  add_footnote("* A negative value indicates that randomisation happened before patients were intubated.") %>%
