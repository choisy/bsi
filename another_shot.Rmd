...
```{r}
required <- c("car", "dplyr", "lubridate", "magrittr", "purrr", "readxl", "stringr", "survival", "tibble", "tidyr")
...
Note that there are 6 missing values in `DOD`.
...
```{r}
pie2 <- function(x) {
  pie(x, cex = 2 / 3, make_italic(names(x)), col = colors[names(x)])
}
```
...
fisher_test <- function(x) {
  fisher.test(matrix(
    c(path_freq[x],
      path_freq_1step[x],
      path_freq_1stCLBSI[x],
      path_freq_1stALBSI[x],
      path_freq_nonCLBSI[x],
      nb_episodes,
      nb_1st_episodes,
      nb_clbsi,
      nb_albsi,
      nb_non_clbsi), 2, byrow = TRUE))
}
```
...
calc_prop <- function(x) {
  list(unlist(prop.test(path_freq[x], nb_episodes)[c("estimate", "conf.int")]),
       unlist(prop.test(path_freq_1step[x], nb_1st_episodes)[c("estimate", "conf.int")]),
       unlist(prop.test(path_freq_1stCLBSI[x], nb_clbsi)[c("estimate", "conf.int")]),
       unlist(prop.test(path_freq_1stALBSI[x], nb_albsi)[c("estimate", "conf.int")]),
       unlist(prop.test(path_freq_nonCLBSI[x], nb_non_clbsi)[c("estimate", "conf.int")])) %>% 
    reduce(rbind) %>% 
    as.data.frame()
}
```
...
Let's apply it for estimate, lower and upper bounds:
...
## 6A. Risk factors associated with BSI (logistic regression)

The following function transform a continuous variable into a categorical
...
The data set to use here:

```{r}
data3 <- data2 %>% 
  select(habsi, Sex, age, hospital, apache, charlson, sepsis, Tetanus, CNS,
         dia_co, Mode, had_catheter, had_arterial, tracheostomy, adm2icu) %>% 
  drop_na() %>% 
  mutate_at("adm2icu", as.numeric) %>% 
  mutate_at(vars(age, adm2icu, apache), makecat)
```

### Numbers of BSI and non-BSI
...
n_and_percentage <- function(var) {
  a <- as.numeric(table(data3[[var]], data3$habsi))
  map2(a, round(100 * a / sum(a), 2), n_and_perc) %>% 
    unlist() %>% 
    matrix(ncol = 2)
}
```
...
```{r}
glm_coef <- function(x, digits = 4) {
  m <- glm(data3$habsi ~ data3[[x]], binomial)
  c <- coef(m)[-1]
  ci <- confint(m)[-1, ]
  if (! is.matrix(ci)) ci <- as.matrix(t(ci))
  cbind(c, ci) %>% 
    exp() %>% 
    round(digits) %>% 
...
```{r}
glm_p <- function(x, digits = 4) {
  glm(data3$habsi ~ data3[[x]], binomial) %>% 
    anova(test = "LRT") %>% 
    extract2("Pr(>Chi)") %>% 
    magrittr::extract(2) %>% 
    round(digits)
...
```{r}
nb_lev <- sapply(variables, function(x) length(unique(data3[[x]])))
```

The blanks we need to add to the outputs:
...
```{r}
m <- glm(habsi ~ ., binomial, data3)
```

```{r}
calc_c_multi <- function(x) {
  x %>% 
    (function(x) map(c(coef, confint), function(f) f(x))) %>% 
    reduce(cbind) %>% 
    (function(y) {if("glm" %in% class(x)) return(magrittr::extract(y, -1, )); y}) %>% 
    exp() %>% 
    round(3) %>% 
    apply(1, put_in_shape) %>% 
    split(rep(seq_along(nb_lev), nb_lev - 1)) %>% 
    map(~ c("", .x)) %>% 
    unlist()
}
```
...
```{r}
cbind(numbers, calc_c_uni(glm_coef), calc_p_uni(glm_p), calc_c_multi(m), calc_p_multi(m)) %>%
  as.data.frame(., row.names(.), stringsAsFactors = FALSE)
```

## 6B. Risk factors associated with BSI (Cox regression)
...
data4 <- data2 %>% 
  select(habsi, Sex, age, hospital, apache, charlson, sepsis, Tetanus, CNS,
         dia_co, Mode, had_catheter, had_arterial, tracheostomy, adm2icu, time2bsi) %>% 
  drop_na() %>% 
  mutate_at("adm2icu", as.numeric) %>% 
  mutate_at(vars(age, adm2icu, apache), makecat)
```
...
```{r}
cox_coef <- function(x, digits = 4) {
  coxph(Surv(time2bsi, habsi) ~ data4[[x]], data4) %>% 
    (function(x) map(c(coef, confint), function(f) f(x))) %>% 
    reduce(cbind) %>% 
    exp() %>% 
    round(digits) %>% 
...
```{r}
m <- coxph(Surv(time2bsi, habsi) ~ ., data4)
```

All together:

```{r}
cbind(numbers, calc_c_uni(cox_coef), calc_p_uni(cox_p), calc_c_multi(m), calc_p_multi(m)) %>% 
  as.data.frame(., row.names(.), stringsAsFactors = FALSE)
```

## 7. Risk factors associated with mortality

The same as above for mortality, with only the Cox regression:

The data:

```{r}
data5 <- data2 %>% 
  select(death90, Sex, age, hospital, apache, charlson, sepsis, Tetanus, CNS,
         dia_co, Mode, had_catheter, had_arterial, tracheostomy, adm2icu, DOD) %>% 
  drop_na() %>% 
  mutate_at("adm2icu", as.numeric) %>% 
  mutate_at(vars(age, adm2icu, apache), makecat)
```
...
```{r}
cox_coef <- function(x, digits = 4) {
  coxph(Surv(data5$DOD, data5$death90) ~ data5[[x]], data5) %>% 
    (function(x) map(c(coef, confint), function(f) f(x))) %>% 
    reduce(cbind) %>% 
    exp() %>% 
    round(digits) %>% 
...
```{r}
m <- coxph(Surv(DOD, as.numeric(death90)) ~ ., data5)
```
...
```{r}
data6 <- data2 %>% 
  select(habsi, DOD, rand2hos, deathICU, death28, death90) %>% 
...
```{r}
kmICU <- survfit(Surv(timeICU, deathICU) ~ 1, data = data6)
plot(kmICU)
```

```{r}
km28 <- survfit(Surv(time28, death28) ~ 1, data = data6)
plot(km28)
```

```{r}
km90 <- survfit(Surv(time90, death90) ~ 1, data = data6)
plot(km90)
```

```{r}
kmICU_2 <- survfit(Surv(timeICU, deathICU) ~ habsi, data = data6)
plot(kmICU_2, col = 3:2)
```

```{r}
km28_2 <- survfit(Surv(time28, death28) ~ habsi, data = data6)
plot(km28_2, col = 3:2)
```

```{r}
km90_2 <- survfit(Surv(time90, death90) ~ habsi, data = data6)
plot(km90_2, col = 3:2)
```

```{r}
with(data6, table(habsi, deathICU))
with(data6, table(habsi, death28))
with(data6, table(habsi, death90))
```
