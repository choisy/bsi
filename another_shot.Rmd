...
if (! "C306" %in% installed) {
  if (! "remotes" %in% installed) install.packages("remotes", repos = "https://cloud.r-project.org")
...
```{r eval = FALSE}
if (! require("kintr")) install.packages("knitr", repos = "https://cloud.r-project.org")
knitr::purl("bsi_analysis.Rmd")
script <- readLines("bsi_analysis.R")
writeLines(script[-(seq_len(which(grepl("starts here", script))),
                    which(grepl("session_info", script)):length(script))],
           "bsi_analysis.R")
```
