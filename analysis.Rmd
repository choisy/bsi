...
# define hospital --------------------------------------------------------------
  dplyr::mutate(hospital = dplyr::recode(sub("-.*$", "", PatientNo),
                                         `003` = "HTD",
                                         `020` = "NHTD",
                                         `103` = "BVTV"))
```
...
data2 %>% 
  mutate(habsi = ! habsi) %>% 
  group_by(hospital, habsi) %>% 
  tally() %>% 
  ungroup() %>% 
  mutate(habsi = recode(as.character(habsi), `FALSE` = "with HABSI", `TRUE` = "without HABSI")) %>% 
  tidyr::pivot_wider(names_from = habsi, values_from = n)
```

```{r eval = FALSE}
v <- "habsi"
data2 %>% 
  mutate(x = {{ rlang::quo_name(v) }} ) %>%
  group_by(hospital, x) %>% 
  tally() %>% 
  ungroup() %>% 
  mutate(x = recode(as.character(x), `FALSE` = paste("with", toupper(v)), `TRUE` = paste("without", toupper(v)))) %>% 
