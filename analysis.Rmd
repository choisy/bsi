...
```{r}
data$DAILY1n2 %>% 
  dplyr::filter(PatientNo == "003-002") %>% 
  dplyr::select(PatientNo, VSDate, CVCCount) %>% 
  print() %>% 
  two_consecutive("name")
```
...
```{r}
for(v in c("habsi", "bsi", "clbsi", "al_bsi")) {
  c("age", "rand2icu", "rand2hos", "adm2icu", "adm2hos", "patdayRand", "patdayHosp") %>% 
