...
The code below defines the variables of interest. Note that the data sets
`MICBLD` and `DAILY1n2` are managed differently than the data sets `FINISH`,
`ENR`, `ADM` and `AIR` because they do not have one row per patient.

```{r}
data2 <- data$FINISH %>%
  dplyr::select(PatientNo, IsCompleted, IsAgree, HosDate) %>% 
...
# selecting variables of interest ----------------------------------------------
  dplyr::select(-IsCompleted, -IsAgree, -HosDate, -Age, -YOB)
```
...
for(v in c("habsi", "bsi", "clbsi_risk", "clbsi", "al_bsi_risk", "al_bsi")) {
  for (g in c("hospital", "Sex", "Source", "nkq", "tracheo")) print(f1(v, g, data3))
}
```
