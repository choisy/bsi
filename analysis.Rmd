---
title: "Blood stream infections in 16HN"
...
# options(width = 260) # console
options(width = 166) # HTML
```

## Introduction

The cleaned data sets used in this analysis are avaible in CSV 
[here](https://raw.githubusercontent.com/choisy/bsi/master/outputs/CSV/bsi.csv) for the
patient-based data (`data2`),
[here](https://raw.githubusercontent.com/choisy/bsi/master/outputs/CSV/micbld.csv) for the
blood microbiology data (`data$MICBLD`) and
[here](https://raw.githubusercontent.com/choisy/bsi/master/outputs/CSV/daily_icu.csv) for
the daily ICU data (`data$DAILY1n2`).

The Excel versions of this data sets are available 
[here](https://www.dropbox.com/sh/bvcn6d7sputb6xi/AAAXe8AHjw3tAYI4JTImGc2Ca?dl=0).
and the outputted tables are available in Word format 
[here](https://www.dropbox.com/sh/ixthouvjkuzzyfo/AAADe4MKGx_yKPaeeXtan8pga?dl=0).
The figures can simply be dragged and dropped (in PNG format) directly from this
HTML document.

The analysis presented here follows
[this document](https://www.dropbox.com/s/7fe0qkgdz05p4rj/BSI%20analysis%20plan_share.docx?dl=0).
The code source used to generate this HTML file is 
[here](https://raw.githubusercontent.com/choisy/bsi/master/analysis.Rmd).
You can download this R markdown file directly from R as so:

```{r eval = FALSE}
download.file("https://raw.githubusercontent.com/choisy/bsi/master/analysis.Rmd", "bsi_analysis.Rmd")
```

You can transforme this `bsi_analysis.Rmd` R markdown document in an R script
named `bsi_analysis.R` as so:

```{r eval = FALSE}
if (! require("kintr")) install.packages("knitr", repos = "https://cloud.r-project.org", dependencies = TRUE)
knitr::purl("bsi_analysis.Rmd")
script <- readLines("bsi_analysis.R")
writeLines(script[-c(seq_len(which(grepl("starts here", script))[1]),
                     which(grepl("session_info", script))[1]:length(script))],
           "bsi_analysis.R")
```

To execute the code (it takes 5-10''), just type:

```{r eval = FALSE}
source("bsi_analysis.R")
```

It will generate this HTML document plus the Word files of the outputed tables.
The session informations are available at the end of this HTML document.

## 0. Packages

Tuning the `install.packages()` function:

```{r starts here}
install_packages <- function(...) install.packages(..., repos = "https://cloud.r-project.org", dependencies = TRUE)
```

Installing the required packages from CRAN:

```{r}
required <- c("car", "dplyr", "flextable", "lubridate", "magrittr", "officer",
              "purrr", "readxl", "stringr", "survival", "tibble", "tidyr", "writexl")
installed <- row.names(installed.packages())
to_install <- setdiff(required, installed)
if (length(to_install)) install_packages(to_install)
```
...
```{r}
if (! require("C306")) {
  if (! require("remotes")) install_packages("remotes")
  remotes::install_github("lampk/C306")
}
```

Here we've used the version of the commit `ad7fd0a8fbdea3a8d7575191609ad5a85d1cc312`.
A mirror of this repository exists [here](https://github.com/choisy/C306).
...
```{r}
yesno2bool <- function(x) na_if(x, "NA") == "Yes"
```

A safe `as_data.frame()` function:

```{r}
as_data.frame <- function(...) as.data.frame(..., stringsAsFactors = FALSE)
```

A `left_join()` by `PatientNo`:
...
```{r}
data_file <- "raw_data/19-8-2019-CTU16HN_Data.xls.xlsx"
```
...
data <- sheets %>%
  ...
  lapply(function(x) mutate_if(x, function(y) "POSIXct" %in% class(y), as.Date)) %>% 
  setNames(sheets)
```
...
Writing `data$MICBLD` to disk:

```{r warning = FALSE}
dir.create("outputs/CSV", recursive = TRUE)
write.csv(data$MICBLD, "outputs/CSV/micbld.csv", quote = FALSE, row.names = FALSE)
```

and 

```{r}
writexl::write_xlsx(data$MICBLD, "outputs/Excel/micbld.xlsx")
```
...
```{r}
clbsi <- leftjoin(first_bsi, data$DAILY1n2) %>% 
...
```{r}
albsi <- leftjoin(first_bsi, data$DAILY1n2) %>% 
...
```{r}
nb_cl_days <- leftjoin(data$DAILY1n2, first_bsi) %>% 
...
```{r}
nb_al_days <- leftjoin(data$DAILY1n2, first_bsi) %>% 
...
Writing `data$DAILY1n2` to disk:

```{r}
write.csv(data$DAILY1n2, "outputs/CSV/daily_icu.csv", quote = FALSE, row.names = FALSE)
```

and 

```{r}
writexl::write_xlsx(data$DAILY1n2, "outputs/Excel/daily_icu.xlsx")
```
...
```{r}
charlson_trinh <- readRDS("Trinh_data/charlson_score.rds")
...
```{r}
apache <- data$APAC %>% 
  leftjoin(select(data2, PatientNo, age, RandDate, Mode, CongIV, Seve, AIDS, Elective, Emergency)) %>% 
...
```{r}
apache_trinh <- readRDS("Trinh_data/apache.ii_score.rds") %>% 
...
Let's add the APACHE-II scores from Trinh:

```{r}
data2 %<>% leftjoin(select(apache_trinh, PatientNo, total_score) %>% rename(apache = total_score))
```

### Adding mortalities from Trinh

```{r}
data2 <- readRDS("Trinh_data/death_day.full.rds") %>% 
  mutate(death28 = Event > 0  & DOD < 29,
         death90 = Event > 0  & DOD < 91) %>%
  select(PatientNo, death28, death90, DOD) %>% 
  leftjoin(select(readRDS("Trinh_data/icu_mortality.full.rds"), -IsTetanus)) %>% 
...
```{r}
data2 <- readRDS("Trinh_data/protocol.VARI.rds") %>% 
...
```{r}
write.csv(data2, "outputs/CSV/bsi.csv", quote = FALSE, row.names = FALSE)
```

and 

```{r}
writexl::write_xlsx(data2, "outputs/Excel/bsi.xlsx")
```
...
patient_day <- function(g, v, df) {
  df %>% 
    ...
    rev(.) %>% 
    c(sum(.)) %>% 
    inset(c("patient-day", rep("", 6)), c(2, 4, 6), .)
}
```
...
day_per_patient <- function(g, v, data) {
  data %>% 
    ...
    inset(c("day/patient", rep("", 6)), c(3, 5, 7), .) %>% 
    inset(c(2, 4, 6), group_by(data, {{ g }}) %>%
                        tally() %>%
                        pull() %>%
                        rev() %>% 
                        c(nrow(data)))
...
make_table_1 <- function(x) {
  ...
  data2 %>% 
    ...
    as_data.frame() %>% 
    set_colnames(c("characteristic",
                   paste(c(rep(trimws(paste(c("non", ""), x)), each = 2),
                           rep("all", 2)), rep(c("n", "summary statistics"), 3)))) %>% 
...
The following function takes an output of `make_table_1()` and writes it to
Word:

```{r}
flex2word <- function(x, file_name = "example.docx", pad = 20, fs = 5) {
  doc <- read_docx()
  ...
  col_keys <- letters[seq_len(length(x))]
  column_names <- names(x)
  
  header2 <- column_names %>% 
    str_replace("VARI ", "") %>% 
    str_replace("non ", "") %>% 
    str_replace("all ", "") %>% 
    inset(1, "") %>% 
  ...
  header1 <- column_names %>% 
    str_replace(" n$", "") %>% 
    str_replace(" summary statistics$", "") %>% 
    setNames(col_keys) %>% 
    as.list()
  
  x %>% 
    set_colnames(col_keys) %>% 
    flextable(c(col_keys, LETTERS[1:3])[c(1, 8, 2:3, 9, 4:5, 10, 6:7)]) %>% 
    delete_part() %>% 
    add_header(values = header2) %>% 
    add_header(values = header1) %>% 
    merge_h(1, part = "header") %>% 
    align(NULL, 1, "left") %>% 
    align(1, NULL, "center", "header") %>% 
    align(2, NULL, "center", "header") %>% 
    hline_top(border = fp_border(width = 1.5), part = "header") %>% 
    hline_bottom(border = fp_border(), part = "header") %>% 
    hline(1, 3:4 , fp_border(), "header") %>% 
    hline(1, 6:7 , fp_border(), "header") %>% 
    hline(1, 9:10, fp_border(), "header") %>% 
    padding(pad_loc, 1, padding.left = pad) %>% 
...
Let's generate of the demographics tables:

```{r warning = FALSE, message = FALSE}
dir.create("outputs/Word", recursive = TRUE)
for (i in c("VARI", "VAT", "VAP", "habsi_risk", "habsi", "ALBSI_risk", "ALBSI",
            "CLBSI_risk", "CLBSI", "Tetanus", "deathICU")) {
  cat(paste("\n********** DEMOGRAPHICS FOR", i, "**********\n\n"))
  make_table_1(i) %>% 
    print() %>% 
    flex2word(paste0("outputs/Word/Table1", i, ".docx"))
}
```
...
```{r}
colors <- setNames(c("#fb8072", "#fdb462", "#80b1d3", "#b3de69", "#8dd3c7",
                     "#ffffb3", "#bebada", "#fccde5", rep("#d9d9d9", 10)),
...
```{r}
make1row <- function(x) {
    map(list(habsi_rate, habsi_prop, nb_cl_days_fct, cl_utilization_ratio, clbsi_rate), function(f) f(x)) %>%
    as_data.frame() %>% 
    unname()
}
```
...
out <- map(groups, make1group) %>% 
  c(list(as_data.frame(c("all", make1row(data2)))), .) %>% 
...
```{r}
(tmp <- lapply(1:7, function(x) c(out[x], inter[x])) %>%
  unlist(FALSE) %>% 
  c(out[8]) %>% 
  bind_rows())
```
...
tmp %>% 
  flextable() %>% 
  align(NULL, 1, "left") %>% 
  padding(seq_len(nrow(tmp))[-c(1, 2, seq(6, 21, 3))], 1, padding.left = 20) %>% 
  fontsize(size = 5, part = "all") %>%
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "outputs/Word/BSIincidence_rate.docx")
```
...
# Making the data:
  data <- data2 %>% 
    select({{event}}, Sex, age, hospital, apache, charlson, sepsis, Tetanus, CNS,
           dia_co, Mode, had_catheter, had_arterial, adm2icu, {{time}}) %>% 
...
risk_factors(habsi, time2bsi) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "outputs/Word/BSI_risk_factors.docx")
```
...
risk_factors(death90, DOD) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "outputs/Word/mortality_risk_factors.docx")
```
...
plotevent <- function(a, ylim = 0:1, col_line = "black", col_area = "grey", add = FALSE) {
  ...
  c(transform_x, transform_y, transform_y, transform_y) %>% 
    ...
    setNames(snames) %>%
    with({
      if (! add) plot(time, 1 - surv, xlab = "time (days)", ylab = "proportion dead", ylim = ylim, type = "n")
...
adjcol <- adjustcolor(colors, .5)
add_legend <- function() {
  legend("topleft", legend = c("HA BSI", "non HA BSI"), fill = adjcol, col = colors, lty = 1, bty = "n", border = NA)
}
```
...
ylim <- c(0, .7)
add_legend <- function() {
  legend("topleft", legend = c("non HA BSI", "CLBSI", "non CL BSI"), fill = adjcol, col = colors, lty = 1, bty = "n", border = NA)
}
```
