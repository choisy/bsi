...
[here](https://raw.githubusercontent.com/choisy/bsi/master/clean_data/CSV/bsi.csv) for the
patient-based data (`data2`),
[here](https://raw.githubusercontent.com/choisy/bsi/master/clean_data/CSV/micbld.csv) for the
blood microbiology data (`data$MICBLD`) and
[here](https://raw.githubusercontent.com/choisy/bsi/master/clean_data/CSV/daily_icu.csv) for
the daily ICU data (`data$DAILY1n2`).

The Excel versions of this data sets are available 
[here](https://www.dropbox.com/sh/lq7dlpfek6t7x5e/AABKWowlibUMCsYqc-JaZQFEa?dl=0).
and the outputted tables are available in Word format 
[here](https://www.dropbox.com/sh/g69zatnfrvgmu9d/AACDg5dMwZgJe8nENaiQelaHa?dl=0).
...
```{r warning = FALSE}
dir.create("clean_data/CSV", recursive = TRUE)
write.csv(data$MICBLD, "clean_data/CSV/micbld.csv", quote = FALSE, row.names = FALSE)
```

and 

```{r}
dir.create("clean_data/Excel")
writexl::write_xlsx(data$MICBLD, "clean_data/Excel/micbld.xlsx")
```
...
```{r}
write.csv(data$DAILY1n2, "clean_data/CSV/daily_icu.csv", quote = FALSE, row.names = FALSE)
```

and 

```{r}
writexl::write_xlsx(data$DAILY1n2, "clean_data/Excel/daily_icu.xlsx")
```
...
```{r}
write.csv(data2, "clean_data/CSV/bsi.csv", quote = FALSE, row.names = FALSE)
```

and 

```{r}
writexl::write_xlsx(data2, "clean_data/Excel/bsi.xlsx")
```
...
for (i in c("VARI", "VAT", "VAP", "habsi_risk", "habsi", "ALBSI_risk", "ALBSI",
  ...
  make_table_1(i) %>% 
    print() %>% 
    flex2word(paste0("outputs/Table1", i, ".docx"))
}
```
...
tmp %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "outputs/BSIincidence_rate.docx")
```
...
risk_factors(habsi, time2bsi) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "outputs/BSI_risk_factors.docx")
```
...
risk_factors(death90, DOD) %>% 
  ...
  autofit() %>% 
  body_add_flextable(doc, .) %>% 
  print(target = "outputs/mortality_risk_factors.docx")
```
